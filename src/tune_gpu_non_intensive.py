...
def train_model(hparams):
    ...
    wandb_logger = WandbLogger(
        log_model=False, # log model at end of training
...
if __name__ == "__main__":
    ...
    for dataset in search_space["dataset"]:
        for batch_size in search_space["batch_size"]:
            '''
            Ignore this part if local training has enough GPU memory >=15.
            Otherwise use this script to train locally and the wikiann script to train on Kaggle or GDRIVE.
            '''
            if dataset == "wikiann" and batch_size == 16:
                 continue
